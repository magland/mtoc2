...
export class Lowerer {
  ...
  private lowerTensorLit(e: Extract<Expr, { type: "Tensor" }>): IRExpr {
    ...
    // Collect exact-fold data when every element is statically known
    // and the total is within the type-level cap. Sign auto-derives
    // from the data inside `tensorDouble`, which lets `sqrt([0 1 4
    // 9])` pass its domain check (sign="nonneg"). The IR still
    // materializes via `mtoc2_tensor_from_row`/`_from_matrix` at
    // codegen time per the always-materialize invariant — the exact
    // data is type-level only.
    let exactData: Float64Array | undefined;
    if (total <= EXACT_ARRAY_MAX_ELEMENTS) {
      const data = new Float64Array(total);
      let allExact = true;
      for (let i = 0; i < total; i++) {
        const elt = loweredFlat[i];
        const v = exactDouble(elt.ty);
        if (v === undefined) {
          allExact = false;
          break;
        }
