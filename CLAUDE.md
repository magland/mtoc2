...
Not yet supported: logical / vector-of-indices indexing (`a(mask)`,
`a(idx_vec)`), member-rooted indexing (`obj.r(1, :, :)`,
`obj.f(i) = rhs`), indexed-delete (`a(2:5) = []`), unknown-shape
constructors (`zeros(n)` where `n` is a runtime-only scalar), general
broadcast (non-scalar mismatched shapes), complex, text concat
(`[a, b]` of two chars), char arithmetic (`'A' + 1`), `strcmp`,
builtin handles, `private/` directories, `@Class/` folders, `import`
